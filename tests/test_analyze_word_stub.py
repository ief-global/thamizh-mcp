"""A bare engine (no sources wired) must stay schema-valid, all-gaps, honest."""
import asyncio
import json
import sys
...
def test_bare_engine_is_schema_valid_and_all_gaps(word):
    ...
    assert {"origin", "lemma", "pos", "meaning", "grammar", "native_equivalent", "formation"} <= gap_fields
    assert a.lemma == "" and a.meaning.senses == [] and a.origin.class_ == "unknown"

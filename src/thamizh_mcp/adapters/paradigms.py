...
The primary FSTs miss a quarter of everyday past-tense forms (போனான், சொன்னான், கொடுத்தான், கற்றான்,
விற்றான், தூங்கினான்) because those lemmas or their irregular past stems are absent from the lexicon.
...
class VerbParadigmAdapter(SourceAdapter):
    ...
    def __init__(self, table_path: Optional[Path] = None):
        ...
        try:
            ...
            self._past = [{**e, "stem": _nfc(e["stem"]), "lemma": _nfc(e["lemma"])}
                          for e in data.get("past", [])]
    ...
    def analyses_for(self, word: str) -> list[MorphAnalysis]:
        """Generate each listed stem × PNG suffix surface form and match the word against it.
        Generation (not prefix-matching) is what makes the Tamil mei+uyir join correct."""
        w = _nfc(word)
        out: list[MorphAnalysis] = []
        for entry in sorted(self._past, key=lambda e: -len(e["stem"])):
            for png, suffix in self._png.items():
                if join_surface(entry["stem"], suffix) == w:
                    out.append(MorphAnalysis(
                        lemma=entry["lemma"], pos="verb",
                        tags=["fin", "sim", f"past={entry['marker']}", f"{png}={suffix}"]))
            if out:
                break   # longest stem wins — don't also match a shorter accidental stem
        return out
